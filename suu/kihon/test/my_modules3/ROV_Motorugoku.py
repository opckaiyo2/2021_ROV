...
class ROV_Motor:
	def __init__(self):
		...
		self.pca.set_pwm_freq(61) #66.4Hz
		#setting
		self.POWER       = 0.5
		self.standard    = 401  #duty 9.8% motor stoped
	...
	try:
		...
		while True:
			...
			if x == 0:
				...
				motor.forward_rotation(1,duty, offset['pin0'])
			elif x == 3:
				motor.forward_rotation(2,duty, offset['pin0'])
			elif x == 4:
				motor.forward_rotation(3,duty, offset['pin0'])
			elif x == 5:
				motor.forward_rotation(4,duty, offset['pin0'])
			elif x == 6:
				motor.forward_rotation(5,duty, offset['pin0'])
			elif x == 7:
				motor.forward_rotation(6,duty, offset['pin0'])
			elif x == 8:
				motor.forward_rotation(7,duty, offset['pin0'])
			else:
				motor.stop()
	except KeyboardInterrupt:
		motor.all_duty0()
