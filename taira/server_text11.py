...
def start():
    ...
    #s.setblocking(False)
    s.bind(('0.0.0.0', 10009))#IPとポート番号を指定します
    offs =40
    offs2=10  #sensui
    duty = 0
    s.listen(1)
    ...
    td = {'mode': 'remote', 'move': 'stop', 'sensui': 'stop','laxis':0.0,'raxis':0.0}

    #text_socket program 以下\
    s2 = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    s2.bind(('0.0.0.0', 1236))  # IPとポート番号を指定します
    s2.listen(1)
    ...
    try:
        ...
        while True:
            ...
            #td['laxis']=duty
            #print(td)
            #clientsocket2.send(bytes("welcome!", 'utf-8'))
            msg2 = pickle.dumps(td)
            clientsocket2.send(bytes("3", 'utf-8'))
            #print('type',type(msg2))
            
            size2=sys.getsizeof(msg2)
            size2=size2+16
            print(td['move'])
            if  td['move']=='for':
                size2=177
            print('size2',size2)
            print('size1',sys.getsizeof(size2))
            strsize=str(size2)
            clientsocket2.send(bytes(strsize,'utf-8'))
            #print(msg2)
            #print(' ')
            #time.sleep(0.001)
            clientsocket2.send(msg2)
            #clientsocket2.send(bytes("i'm good at sending!", 'utf-8'))
            #clientsocket2.send(bytes("welcome!", 'utf-8'))
